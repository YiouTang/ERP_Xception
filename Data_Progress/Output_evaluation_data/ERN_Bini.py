...
if __name__ == '__main__':
    ...
    for k in range(len(datalist)):
        ...
        data0 = pickle_read(
            'F:\\研究生文件\\小论文2相关资料\\开源代码\\第二篇小论文+大论文相关内容\\DataProgress\\计算距离矩阵_gpt优化\\ERN中间数据\\bini\\按列为时间步\\column_data0' + d1path + '.pickle');
        distance0 = pickle_read(
            'F:\\研究生文件\\小论文2相关资料\\开源代码\\第二篇小论文+大论文相关内容\\DataProgress\\计算距离矩阵_gpt优化\\ERN中间数据\\bini\\按列为时间步\\column_distance0' + d1path + '.pickle');
        data1 = pickle_read(
            'F:\\研究生文件\\小论文2相关资料\\开源代码\\第二篇小论文+大论文相关内容\\DataProgress\\计算距离矩阵_gpt优化\\ERN中间数据\\bini\\按列为时间步\\column_data1' + d1path + '.pickle');
        distance1 = pickle_read(
            'F:\\研究生文件\\小论文2相关资料\\开源代码\\第二篇小论文+大论文相关内容\\DataProgress\\计算距离矩阵_gpt优化\\ERN中间数据\\bini\\按列为时间步\\column_distance1' + d1path + '.pickle');
        ...
        pbar.update(1)  # 进度条更新
        pickle_write(
            'F:\\研究生文件\\小论文2相关资料\\开源代码\\第二篇小论文+大论文相关内容\\DataProgress\\输出平均数据_gpt优化\\ERN输出数据\\bini\\按列为时间步\\' + d1path + '.pickle',
            imgs)
        imgs = []
    pbar.close()  # 关闭资源
