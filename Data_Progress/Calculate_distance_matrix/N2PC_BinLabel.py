...
if __name__ == '__main__':
    imgs = []
    path = "F:\\研究生文件\\小论文2相关资料\\实验数据\\N2PC\\binlabelLabel\\"  # 设置路径
    ...
    for k in range(len(datalist)):
        ...
        for l in range(labeltensors.shape[0]):
            if (int(labeltensors[l]) == 3):
                data0.append(datatensors[l].float())
            if (int(labeltensors[l]) == 4):
                data1.append(datatensors[l].float())
            if (int(labeltensors[l]) == 5):
                data2.append(datatensors[l].float())
            if (int(labeltensors[l]) == 6):
                data3.append(datatensors[l].float())
        ...
        distance2 = manager.list(np.zeros((len(data2), len(data2))))  # 注册线程安全的二维数组
        distance3 = manager.list(np.zeros((len(data3), len(data3))))  # 注册线程安全的二维数组

        data0, distance0=data_processing(data0,distance0)
        data1, distance1 = data_processing(data1, distance1)
        data2, distance2 = data_processing(data2, distance2)
        data3, distance3 = data_processing(data3, distance3)

        distance0 = np.asarray(distance0)
        distance1 = np.asarray(distance1)
        distance2 = np.asarray(distance2)
        distance3 = np.asarray(distance3)
        pickle_write('column_data0'+d1path + '.pickle', data0)
        pickle_write('column_data1'+d1path + '.pickle', data1)
        pickle_write('column_data2'+d1path + '.pickle', data2)
        pickle_write('column_data3'+d1path + '.pickle', data3)
        pickle_write('column_distance0' +d1path + '.pickle', distance0)
        pickle_write('column_distance1' +d1path + '.pickle', distance1)
        pickle_write('column_distance2' +d1path + '.pickle', distance2)
        pickle_write('column_distance3' +d1path + '.pickle', distance3)
